...
from ..database.orm_crud import create, exists, get_one, list_items, update
...
async def create_resource_grant(
    session: AsyncSession | async_scoped_session[AsyncSession],
    request: CreateResourceGrantRequest,
) -> ResourceGrant:
    filters = _resource_filters(request)
    for _attempt in range(_CAS_ATTEMPTS):
        existing = await get_one(session, MCPResourceGrant, filters)
        if existing is None:
            model = await create(
                session,
                MCPResourceGrant,
        ...
        if affected > 0 or not rowcount_known:
            model = await get_one(
                session, MCPResourceGrant, {"grant_id": existing.grant_id}
            )
            if (
                model is not None
...
async def has_any_active_grant(
    session: AsyncSession | async_scoped_session[AsyncSession],
    *,
    principal_id: str,
) -> bool:
...
async def list_active_resource_grants(
    session: AsyncSession | async_scoped_session[AsyncSession],
    *,
    principal_id: str,
) -> tuple[ResourceGrant, ...]:
    """List one principal's active exact grants in stable identifier order."""
    models = await list_items(
        session,
        MCPResourceGrant,
        {"principal_id": principal_id, "revoked_at": None},
        order_by=["grant_id"],
    )
    return tuple(_grant_contract(model) for model in models)
...
async def revoke_resource_grant(
    ...
    for _attempt in range(_CAS_ATTEMPTS):
        existing = await get_one(session, MCPResourceGrant, {"grant_id": grant_id})
        ...
        if not rowcount_known:
            model = await get_one(session, MCPResourceGrant, {"grant_id": grant_id})
            if (
                model is not None
                and model.revision == next_revision
