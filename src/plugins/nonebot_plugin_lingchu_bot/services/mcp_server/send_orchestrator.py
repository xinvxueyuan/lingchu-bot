...
class SendPolicy(Protocol):
    async def authorize_resource(
        self, authenticated: Any, **kwargs: Any
...
class SendMessageOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self._principal_limiter = _Limiter(20, clock=clock)
        self._conversation_limiter = _Limiter(6, clock=clock)
    ...
    async def send(
        ...
        try:
            ...
            raise
        principal_sem = self._semaphore(self._principal_sem, principal_id, 8)
        conversation_sem = self._semaphore(self._conversation_sem, conversation_key, 2)
