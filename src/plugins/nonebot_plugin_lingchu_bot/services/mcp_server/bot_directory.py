...
class BotDirectory:
    ...
    def list_connected(
        self,
        allowed: frozenset[BotAddress],
    ) -> tuple[ConnectedBotSummary, ...]:
        """List online bots whose complete addresses are explicitly allowed."""
        summaries = (
            connected.summary
            for address, connected in self._connected.items()
        ...
        return tuple(
            sorted(
                summaries,
                key=lambda item: (
                    item.address.platform_id,
                    item.address.adapter_id,
                    item.address.protocol_id,
                    item.address.bot_id,
                ),
            )
        )
