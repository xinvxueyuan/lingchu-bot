...
async def test_quit_group_sends_message_and_calls_api(
    ...
    with (
        patch.object(quit_group_cmd, "send") as mock_send,
        patch.object(quit_group_cmd, "finish") as mock_finish,
    ):
...
@pytest.mark.asyncio
async def test_quit_group_handles_network_error(
    mock_bot: MagicMock, mock_event: MagicMock
) -> None:
    ...
    with (
        patch.object(quit_group_cmd, "send"),
        patch.object(quit_group_cmd, "finish") as mock_finish,
    ):
        await milkybot_quit_group(bot=mock_bot, event=mock_event)

    assert "退出群失败，网络异常" in str(mock_finish.call_args)


@pytest.mark.asyncio
async def test_quit_group_handles_action_failed_error(
    mock_bot: MagicMock, mock_event: MagicMock
) -> None:
    ...
    assert "退出群失败，操作被拒绝" in str(mock_finish.call_args)
