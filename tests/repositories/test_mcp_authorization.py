...
@pytest.mark.asyncio
async def test_create_principal_enforces_issuer_subject_identity() -> None:
    ...
    with (
        ...
        result = await repo.create_service_principal(request)
    assert result.principal_id == "principal-1"
    assert get_mock.call_args.args[1] == {
        "issuer": "https://issuer.example",
...
@pytest.mark.asyncio
async def test_resolve_principal_preserves_disabled_state() -> None:
...
@pytest.mark.asyncio
async def test_same_issuer_and_value_use_separate_identity_kind_namespaces() -> None:
...
@pytest.mark.asyncio
async def test_find_grant_requires_every_exact_resource_component() -> None:
...
@pytest.mark.asyncio
async def test_has_any_active_grant_uses_principal_and_active_predicate() -> None:
    exists_mock = AsyncMock(return_value=True)
    with patch.object(repo, "exists", exists_mock):
        result = await repo.has_any_active_grant(principal_id="principal-1")
...
@pytest.mark.asyncio
async def test_regrant_revoked_resource_increments_revision() -> None:
    ...
    with (
        ...
        result = await repo.create_resource_grant(_grant_request())
    assert result.revision == 5
    assert update_mock.call_args.args[2]["revision"] == 5
    assert update_mock.call_args.args[2]["revoked_at"] is None


@pytest.mark.asyncio
async def test_revocation_increments_revision_and_invalidates_match() -> None:
    ...
    with (
        ...
        result = await repo.revoke_resource_grant("grant-1")
    assert result is True
    assert update_mock.call_args.args[2]["revision"] == 3
    assert update_mock.call_args.args[2]["revoked_at"] is not None


@pytest.mark.asyncio
async def test_revocation_retries_when_revision_compare_and_swap_loses() -> None:
    ...
    with (
        ...
        assert await repo.revoke_resource_grant("grant-1") is True
    assert update_mock.call_args_list[0].args[1]["revision"] == 2
    assert update_mock.call_args_list[1].args[1]["revision"] == 3


@pytest.mark.asyncio
async def test_revocation_unknown_rowcount_never_assumes_success() -> None:
    ...
    with (
        ...
        assert await repo.revoke_resource_grant("grant-1") is False
    assert update_mock.await_count == 3


@pytest.mark.asyncio
async def test_regrant_unknown_rowcount_requires_matching_revision_and_state() -> None:
    ...
    with (
        ...
        await repo.create_resource_grant(_grant_request())
    assert update_mock.await_count == 3
