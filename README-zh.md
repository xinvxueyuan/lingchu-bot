...
- [开发指南](apps/docs/content/docs/developer-guide/architecture/introduction.mdx)
...
`LINGCHU_SUPERUSERS` 定义 Lingchu 基于 UID 的超级用户账号，并按 JSON5 解析：

```dotenv
LINGCHU_SUPERUSERS='{"userA":{"qq":123,"telegram":"tg-id","discord":"discord-id"}}'
```

当未设置 `LINGCHU_SUPERUSERS` 时，Lingchu 会兼容导入 NoneBot 原生
`SUPERUSERS`，将其作为 QQ 账号并按顺序生成 `user1` 到 `user999`。如果两个来源
都不存在、值不是 UID 到平台账号 ID 的映射，或同一平台账号绑定到多个 UID，启动会
失败。
