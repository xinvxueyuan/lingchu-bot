...
| 运行配置 | localstore 配置目录下的插件专属 JSON5 文件，并可由更高优先级的 NoneBot / 环境变量覆盖。 |
...
| LLM 服务 | 通过 `AI_*` 设置提供的可选 OpenAI / LiteLLM 聊天命令。 |
...
| NoneBot 核心 | `SUPERUSERS` | `LINGCHU_SUPERUSERS` 缺失或为 null 时的 QQ 账号 fallback。 |
...
| Lingchu 运行时 | `LINGCHU_LOCALE` / `LC_LOCALE` / `LOCALE` | 运行时语言；可用目录为 `zh_CN` 和 `en_US`。 |
...
| LLM 服务 | `AI_PROVIDER` | LLM 后端：`litellm` 或 `openai`。 |
| LLM 服务 | `AI_MODEL` | 模型 id（默认 `gpt-4o-mini`）。 |
| LLM 服务 | `AI_BASE_URL` | 可选的 OpenAI 兼容 base URL。 |
| LLM 服务 | `AI_TIMEOUT` | 请求超时秒数。 |
| LLM 服务 | `AI_API_KEY` | API 密钥；聊天命令必需。 |
