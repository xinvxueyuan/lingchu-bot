...
This project is indexed by GitNexus as **lingchu-bot** (5475 symbols, 10007 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
Use this file as the Claude Code mirror of `AGENTS.md`. It keeps the same shared CREATE structure and adds only the Claude-specific behavioral guidelines at the end.
...
- CircleCI side (`.circleci/`) uses no orbs; cimg images are pinned by minor tag (`cimg/python:3.13.15`, `cimg/node:24.19`); uv and Task are installed from their official installers.
...
- Release branches switched from literal `releases/X.Y.Z` to bump-style `releases/<bump-name>` (`major`/`minor`/`patch`/`stable`/`alpha`/`beta`/`rc`) so the release workflow can reuse `task ci:version:bump` from `versioned-build`. The actual version is computed from the latest tag + `uv version --bump`, then a `ci:version:write-config` step syncs `core/config.py` + `package.json` before the tag is created.
- `release.yml::validate` derives `BUMP_LEVEL` / `BUMP_PRERELEASE` from the branch name via the shared `task ci:version:derive-bump` task (delegating to `scripts/ci_derive_bump.py`), the same single source of truth that `ci-builds.yml::versioned-build`, `release:prepare`, and `release:notes` consume. `task ci:version:bump` is called with `DRY_RUN=true` from `release:prepare` / `release:notes` so local scaffolding does not mutate `pyproject.toml` (the task runs `git checkout -- pyproject.toml` after the bump); the `build` job amends the release branch if the CI-computed version drifts from the locally-written one so the tag and source files stay in sync. `workflow_dispatch` accepts a `bump` choice input rather than a literal version string.
- Release notes are still keyed by computed version (`.github/releases/<version>.md`); the developer scaffolds the file with `task release:notes` (or the `release:prepare` hook) before pushing. At release time, the `github-release` job merges the manual notes with GitHub's auto-generated changelog (generate-notes API, called after the tag is created) into the release body — manual notes on top, auto PR/commit changelog below a `---` separator. The merge step also replaced the old cross-job `RUNNER_TEMP` body_path reference (job temp dirs are isolated, so that body file never existed on the release runner).
...
Do not jump straight into implementation for non-trivial changes. First understand the request, inspect related files, identify edge cases, and choose the smallest coherent approach.

### Simplicity First

Prefer straightforward code over speculative abstraction. Add abstractions only when they reduce real duplication or match an existing project pattern.

### Surgical Changes

Keep edits scoped to the requested behavior and nearby ownership boundaries. Do not refactor unrelated code, rename unrelated symbols, or churn formatting outside the touched area.

### Goal-Driven Execution

Once the goal is clear, carry the work through implementation, targeted verification, and a concise summary. If blocked, state the concrete blocker and the evidence gathered.
