...
This project is indexed by GitNexus as **lingchu-bot** (7926 symbols, 14152 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- Release notes are still keyed by computed version (`.github/releases/<version>.md`); the release workflow reads the file after `ci:version:bump` produces the version, and the developer scaffolds the file with `task release:notes` (or the `release:prepare` hook) before pushing.
