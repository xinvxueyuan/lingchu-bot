...
- External APIs (OneBot, Milky, QQ adapters)
...
### Project-Specific Mocking

- OneBot V11 adapter returns `dict` for API responses; mock with `{}`
- Milky adapter returns Model objects; mock with `MagicMock(card="", nickname="")`
- Command handler tests use `patch.object(matcher, "finish")` / `patch.object(matcher, "send")` instead of path-based mocking
