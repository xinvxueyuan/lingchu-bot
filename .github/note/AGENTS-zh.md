...
- **手动触发发布**：正式版本仅手动触发 —— `release.yml` 通过 `workflow_dispatch` 的 `bump` 输入运行（不再推送 `releases/<bump>` 分支）。开发者在触发分支（`main`）提交同步后的版本，然后运行 `task release:publish BUMP=<bump>`（即 `gh workflow run release.yml -f bump=<bump>`）。触发前必须保持 `pyproject.toml`、`package.json` 和 `core/config.py` 中的版本一致。
...
- 发布仅手动触发：`release.yml` 通过 `workflow_dispatch` 的 `bump` 输入运行（`major`/`minor`/`patch`/`stable`/`alpha`/`beta`/`rc`），不再推送 `releases/<bump>` 分支。实际版本由最新 tag + `uv version --bump` 推导，再由 `task ci:version:write-config` 把 `core/config.py` + `package.json` 同步后打 tag。开发者在当前分支（`main`）用 `task release:prepare BUMP=<bump>` 准备发布，提交版本文件与 notes 后，用 `task release:publish BUMP=<bump>` 触发。`ci-builds.yml::versioned-build` 改为**每日定时**（北京时间 02:00，UTC `0 18 * * *`）运行、不再随推送触发 —— 因此 `dev*` 推送和 `main` 上的发布准备提交都不会被自动打 dev 标签，发布历史保持线性。
