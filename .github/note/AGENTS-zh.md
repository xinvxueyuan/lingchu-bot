...
- **手动触发发布**：正式版本仅手动触发 —— `release.yml` 通过 `workflow_dispatch` 的 `bump` 输入运行（不再推送 `releases/<bump>` 分支）。发布版本**完全由工作流用 `uv version --bump` 从最新 tag 推导**（与每日 dev 构建同一 `ci:version:bump` 链路）；开发者不写版本文件，只需用 `task release:prepare BUMP=<bump>` 脚手架 `.github/releases/<version>.md`、更新 `CHANGELOG.md`，提交到 `main` 后用 `task release:publish BUMP=<bump>`（即 `gh workflow run release.yml -f bump=<bump>`）触发。工作流会把推导出的版本文件提交回 `main` 并在同步后的提交上打 tag，保证 tag 与源码一致。
...
- CircleCI 侧（`.circleci/`）不使用 orb，唯一例外是认证的 `codecov/codecov` orb（版本固定 `@6.0.0`，用于覆盖率上传）；cimg 镜像按 minor 标签固定（`cimg/python:3.13.15`、`cimg/node:24.19`）；uv 与 Task 由官方安装器安装。
- 版本验证系统：分支名约定（`dev-minor-*`/`dev-major-*`/`dev-alpha-*`/`dev-beta-*`/`dev-rc-*`/`dev-stable-*`）驱动 `ci:version:bump` 中的 `BUMP_LEVEL`/`BUMP_PRERELEASE`。`ci:version:precheck` 校验 PEP 440 + 大于所有 tag + 源一致性 + 无重复 tag。`ci:version:postcheck` 调用 `release:verify-version` + dev release 语义。uv `--bump` 参数序列由 `scripts/ci_version_bump_args.py` 派生 —— 基于 uv 官方文档（`docs.astral.sh/uv/guides/package/#updating-your-version`）并经 uv CLI 实测验证的纯 Python 决策表，覆盖全部边界情况：稳定版→预发布需 level+pre；同类预发布继续循环（`1.0.1a1`+alpha→`1.0.1a2`）；升级转换同 base（`a`→`b`→`rc`）；降级转换需 level（`rc`→`a`→`1.0.2a1`）；预发布→dev 需 level；预发布→stable 清除预发布，且非 patch 的 level 同时递增 release 组件（`0.6.4.dev3`+minor+stable→`0.7.0` 一步完成）。
- 发布仅手动触发：`release.yml` 通过 `workflow_dispatch` 的 `bump` 输入运行（`major`/`minor`/`patch`/`stable`/`alpha`/`beta`/`rc`），不再推送 `releases/<bump>` 分支。版本**完全由 `uv version --bump` 从最新 tag 推导**（与每日 dev 构建同一 `ci:version:bump` 链路），开发者不写版本文件。开发者用 `task release:prepare BUMP=<bump>` 脚手架 `.github/releases/<version>.md`、更新 `CHANGELOG.md`，提交到 `main` 后用 `task release:publish BUMP=<bump>` 触发；`build` 作业把推导出的版本文件写入并提交到 `main`，`github-release` 作业在同步后的提交上打 tag，保证 tag 与源码一致。`ci-builds.yml::versioned-build` 改为**每日定时**（北京时间 02:00，UTC `0 18 * * *`）运行、不再随推送触发 —— 因此 `dev*` 推送和 `main` 上的发布提交都不会被自动打 dev 标签，发布历史保持线性。
- `release.yml::validate` 通过共享的 `task ci:version:derive-bump` 任务（委托给 `scripts/ci_derive_bump.py`）从 `workflow_dispatch` 的 `bump` 输入推导 `BUMP_LEVEL` / `BUMP_PRERELEASE`，与 `ci-builds.yml::versioned-build`、`release:prepare`、`release:notes` 共享同一份单一来源。`task ci:version:bump` 在 `release:prepare` / `release:notes` 中以 `DRY_RUN=true` 调用，因此本地脚手架不会污染 `pyproject.toml`（任务在 bump 后执行 `git checkout -- pyproject.toml`）；`build` job 在 CI 计算的版本与本地写入版本出现漂移时会把版本文件同步到触发分支，使 tag 与源码保持一致。
- 发布说明仍按计算出的版本号命名（`.github/releases/<version>.md`）；开发者通过 `task release:prepare BUMP=<bump>`（或 `task release:notes BUMP=<bump>`）脚手架该文件，并在触发前提交到触发分支。发布时 `github-release` job 会把手工 notes 与 GitHub 自动生成的变更说明（generate-notes API，在 tag 创建后调用）合并为 release body —— 手工 notes 在上、自动 PR/commit 变更列表在下，中间以 `---` 分隔。该合并步骤同时修复了原跨 job 引用 `RUNNER_TEMP` body_path 的隐患（不同 job 的临时目录相互隔离，原 body 文件在 release runner 上并不存在）。
...
- CircleCI 双线看门狗：`.circleci/config.yml`（setup pipeline；每小时兜底由项目级 Schedule Trigger 提供——setup config 不支持 `triggers: schedule`）运行 `preflight` 初检 job——先 sleep 15s 给 GitHub Actions 启动窗口，再通过 `scripts/ci_gh_preflight.py` 查询 head commit 的 `actions/runs`；仅当 GitHub Actions 全部失败/无记录时才以 `.circleci/checks.yml` 继续全量检查（镜像 python.yml 静态分析 + 7 单元 DB 测试矩阵 + frontend.yml 文档检查）；GH 健康（queued/in_progress/success/skipped）时以 no-op 继续。缺少 `GITHUB_TOKEN`（手动配置的 Project Settings 环境变量，仅需 `actions: read`）或 GitHub API 出错时 fail-open 走全量检查；fork PR 无 secret 亦 fail-open。构建/发布/部署仍仅由 GitHub Actions 负责。CircleCI 无 cancel-in-progress 等价物。
- Codecov 覆盖率上传仅来自 SQLite 单元（每个 commit 一份代表性报告）：GH 侧在 python.yml tests job 用 `codecov/codecov-action@v7.0.0`（SHA 固定）；CircleCI 侧在 tests-sqlite job 用 `codecov/codecov@6.0.0` orb。`CODECOV_TOKEN` 作为 GitHub repo secret 与 CircleCI 项目环境变量存放。Codecov 自身的数值可能因报告处理与 pytest 的 `--cov-fail-under` 门槛略有差异。
