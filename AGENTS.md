...
pnpm --filter docs run test:e2e:hook             # Playwright Chromium smoke test
pnpm --filter docs run test:e2e                  # Playwright all configured browsers
```
...
│        └── onebot11/napcat/ ────────────► OneBot extensions      │
└──────────────────────────────────────────────────────────────────┘
```
...
| Test docs E2E | `pnpm --filter docs run test:e2e:hook` |
...
### Pre-commit Hook Auto-Staging Side Effects

The `pre-commit` hook in this repo runs GitNexus Analyzer and `end-of-file-fixer`, both of which can introduce **unintended changes** to the working tree during a commit:

1. **GitNexus auto-stages `AGENTS.md` and `CLAUDE.md`**: When the analyzer detects new symbols, it updates the GitNexus block in `AGENTS.md`/`CLAUDE.md` and auto-stages the change. The pre-commit output prints: `ℹ️  检测到 Gitnexus 更新了 AGENTS.md CLAUDE.md，正在暂存...`. This means the staged set may include files you did not explicitly `git add`, and the resulting commit's diff scope may surprise reviewers.

2. **`end-of-file-fixer` modifies files**: When `trim trailing whitespace` or `fix end of files` detects violations, it auto-fixes the files and re-stages them. For `.po`/`.pot` files generated by Babel and for `.turbo/preferences/tui.json` (created by `pnpm turbo run`), the missing-trailing-newline fix may inadvertently add `*.json`, `*.po`, or `.turbo/*` to your commit.

3. **`pnpm turbo run` creates `.turbo/preferences/tui.json`**: The TUI mode of turbo writes a `preferences/tui.json` file with `is_task_list_visible` and `active_task` fields. This is a **user-local** state file (it records the last turbo task the user ran) and is NOT in `.gitignore` by default. If `pre-commit` runs `turbo run check-types`, this file is created and then `end-of-file-fixer` may add a trailing newline, causing the file to appear as modified in `git status`.

**Mitigations:**

- After `git commit` completes, re-run `git status` to confirm only expected files were committed. If unexpected files appear, use `git reset --soft HEAD~1` to undo the commit, then `git reset HEAD <unwanted-file>` to unstage, and re-commit.
- Add `.turbo/` to `.gitignore` to prevent accidental commits of turbo state files.
- Run `ruff format <file>` or `uv run -m ruff format .` **before** the first commit in a series that touches i18n or test files — `ruff format --check` failures will block the commit and may be a symptom of stale state from a previous hook run.

**Key lesson**: Pre-commit hooks can change the staged set. Always verify the commit's `git diff --stat` matches your intent before pushing.

### Remote Branch Cleanup After Merge

When deleting a feature branch after merging to `main`, **verify whether the remote branch exists before attempting `git push origin --delete`**:

- Run `git ls-remote origin | grep <branch-name>` first. If the branch was never pushed (e.g., a local-only feature branch or one already deleted on the remote), `git push origin --delete` will fail with `remote ref does not exist`.
- Use `git branch -a` to inspect both local and remote-tracking branches. A branch listed only under the unstarred (non-`*`) entries is local-only.
- The correct sequence after merge is: `git checkout main` → `git merge --ff-only <branch>` → `git branch -d <branch>` (local delete) → `git push origin main` (push the merge) → `git push origin --delete <branch>` (only if remote branch exists).

Key lesson: don't blindly run `git push origin --delete` — always check `git ls-remote` first to avoid spurious errors.
