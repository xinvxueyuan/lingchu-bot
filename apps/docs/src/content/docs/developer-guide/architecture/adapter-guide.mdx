...
## Telegram platform profile

Telegram is selected explicitly with `LINGCHUAdapter=~telegram` and uses the
...
Long polling is the default. Webhook mode requires an HTTPS endpoint and the
...
<Aside type="caution" title="Startup failure">
