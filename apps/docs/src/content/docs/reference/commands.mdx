...
The chat command requires an explicit `[pydantic-ai]` section in `llm.toml`.
Set the provider-prefixed `model` (for example `openai:gpt-5.2`),
`base_url`, `timeout`, and `api_key_env` there. Store secrets in environment
variables such as `LINGCHU_AI_API_KEY`, then reference the variable name with
`api_key_env`.
...
then enrich the prompt with optional TIPO before generating one
...
The same managed LLM no longer performs provider-native web research after the
Pydantic AI migration. Native web search was a LiteLLM-only capability and is
now unavailable; image generation proceeds using the LLM-produced description
and tags directly.
