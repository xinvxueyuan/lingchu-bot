...
<Aside title="Version requirement">
...
For adapter configuration in `.env`, driver requirements, and connection setup, see [Adapter system](./architecture/adapter-system). For how stored records derive their `platform` field from this registry, see [Message Store](./architecture/message-store).
