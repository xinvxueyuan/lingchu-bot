...
| JSON Schema files | Generated manually with pydantic `model_json_schema()` | pydantic owns JSON Schema emission; startup never installs them. |
...
3. Run `task db:check`; regenerate editor schemas via pydantic when they are needed.
