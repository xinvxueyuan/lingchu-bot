...
If you only need to re-scaffold the notes file (e.g., after previewing the version), run `task release:notes BUMP=stable`. It calls `ci:version:bump DRY_RUN=true` and creates the file if it is missing.
...
| Job | Responsibility |
| --- | --- |
| `validate` | Derive `BUMP_LEVEL` / `BUMP_PRERELEASE` from the `bump` dispatch input; run `ci:version:bump` (uv, from latest tag) → `precheck` → `write-config` → `postcheck`; locate `.github/releases/<version>.md`; expose the computed version to downstream jobs. |
