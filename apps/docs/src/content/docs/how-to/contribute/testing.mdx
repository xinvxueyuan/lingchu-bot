...
- `👷 CI-builds`: runs `task ci:build`; on pushes to `dev*` (and manually via `workflow_dispatch`), it also syncs the version from the latest tag, bumps the dev version, writes `core_version`, builds, archives artifacts, generates provenance, and tags the version.
