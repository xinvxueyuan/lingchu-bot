...
- `is_profile_enabled(profile)` 与 `iter_enabled_profiles()` 报告哪些平台 profile 拥有已启用的适配器。权限层使用 `iter_enabled_profiles()`，因此只有实际使用的平台权限模块才会被导入与种子。
...
关于 `.env` 中的适配器配置、驱动要求与连接建立，参见[适配器系统](./architecture/adapter-system)。关于存储记录如何从本注册表派生其 `platform` 字段，参见[消息存储](./architecture/message-store)。
