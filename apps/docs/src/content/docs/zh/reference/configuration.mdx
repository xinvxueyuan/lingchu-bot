...
`model` 字段必须匹配 `^[\w.-]+:[\w./-]+$`；前缀选择 Pydantic AI 提供商（如
...
- HTTP 出站层仍须重新校验 DNS 结果与重定向，因为静态配置校验不会执行阻塞式 DNS
  解析。`[pydantic-ai]` 段不会对 `base_url` 做 SSRF 校验；请在部署出站边界
  强制网络策略。
- 普通 LLM 调用只把模型工具调用作为数据返回，绝不执行。只有 `llm.toml` 中
  `[mcp]` 配置的显式审查 Agent 可以执行 MCP；它要求身份组预授权，并将多轮工具
  循环委托给一个配置了已授权 `MCPToolset` 的 `pydantic_ai.Agent`。旧版项目侧
  审查步骤与确认流程已移除。设置 `enabled = false` 会禁用全部 MCP 发现与执行。
