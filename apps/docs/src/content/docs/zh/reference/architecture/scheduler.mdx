...
`database/models/scheduler.py` 在 `lingchu_scheduled_jobs` 表上定义 `ScheduledJob` ORM 模型。它使用 `database/_dialect_compat.py` 中的跨方言兼容类型（`compat_string`、`CompatText`、`CompatBoolean`、`CompatDateTimeTZ`），使同一套 schema 可在 SQLite、PostgreSQL、MySQL 与 MariaDB 上工作。
