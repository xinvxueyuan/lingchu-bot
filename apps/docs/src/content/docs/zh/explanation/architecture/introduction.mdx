...
<Badge variant="accent">~onebot.v11</Badge> 启用中 <Badge variant="destructive">~milky</Badge> <Badge variant="destructive">~qq</Badge> <Badge variant="destructive">~onebot.v12</Badge> 已移除
...
关于详细的适配器配置和 API 差异，参见[适配器系统](../../reference/architecture/adapter-system)。关于平台、协议和实现专属的命令参考，参见[平台](../platforms)。
