...
3. 验收标准：进程到达 `Application startup complete.` 且无硬错误。`LLM configuration missing or invalid; AI is unavailable` 告警可接受（之后运行 `lingchu config init` 种子化 `llm.toml`）；任何 traceback、schema 写入或迁移副作用都是硬错误。

### 阶段 3 — CLI 工具验证

验证 `lingchu` CLI 在开发环境和打包分发两种形态下均可用。

**开发环境** — 通过 `uv` 直接调用源码 CLI：
...
`lingchu doctor` 应对 `nonebot2`、`nonebot-adapter-onebot`、`nonebot-plugin-localstore`、`nonebot-plugin-lingchu-bot` 报告 `ok`。任何 `missing` 状态都是硬错误。

**生产封包检查** — 验证构建出的 wheel 暴露正确的 entry point 且在隔离环境中行为正常：

```bash
task ci:wheel-smoke
```

该任务运行 `scripts/check-wheel-entrypoints.py`，构建 wheel 后在一个临时项目中（剥离 `LINGCHU_*`、`LOCALSTORE_*`、`ENVIRONMENT` 环境变量）断言：

- wheel 声明了 `lingchu = "_lingchu_bot_cli.app:main"` 和 `nb lingchu = "_lingchu_bot_cli.nb_plugin:install"` entry point；
- `Typer` 被声明为运行时依赖，`nb-cli` 不是；
- `lingchu --help`、`nb lingchu --help`、`nb lingchu doctor --json` 成功；
- `nb lingchu config validate --path missing.toml` 保留 Typer 域错误（退出码 1 且 stderr 包含 `missing configuration file`）；
- `lingchu config init`、`lingchu config validate`、`lingchu schema install` 正确往返，不向项目 cwd 写入文件或残留 `.tmp` 文件。
...
- `smoke-test` 在 `👷 CI-builds` 的 `build` job 之后运行，仅在构建成功时触发。
- `docs-smoke` 在 `📚 Docs Deploy` 的 docs build 之后运行，仅在文档构建成功时触发。
...
项目通过 `SQLALCHEMY_DATABASE_URL` 环境变量支持 6 个数据库后端测试，遵循 [NoneBot 数据库测试最佳实践](https://nonebot.dev/zh/best-practice/database/developer/test)。
...
- **Oracle**：使用 `ghcr.io/gvenzl/oracle-free:23-slim` 镜像（GitHub Container Registry，避免 Docker Hub 速率限制）；需要 Oracle 12.2+（128-char 标识符 + `MERGE INTO`）。
...
- **PostgreSQL 16**（`postgres:16-alpine`，支持至 2028-11）与 **PostgreSQL 18**（`postgres:18-alpine`，最新版）
- **MySQL 8.4 LTS**（`mysql:8.4`，支持至约 2032）与 **MySQL 9.7 LTS**（`mysql:9.7`，支持至约 2034）
- **MariaDB 11.4 LTS**（`mariadb:11.4`，支持至 2029-05）与 **MariaDB 11.8 LTS**（`mariadb:11.8`，支持至 2028-06）
- **Oracle 23ai**（`ghcr.io/gvenzl/oracle-free:23-slim`，来自 GitHub Container Registry；唯一免费版本，无 LTS 拆分）
- **SQL Server 2022** 与 **SQL Server 2025**（`mcr.microsoft.com/mssql/server:2022-latest` / `2025-latest` + apt 安装 `msodbcsql18`；已从废弃的 `azure-sql-edge` 镜像迁移）
