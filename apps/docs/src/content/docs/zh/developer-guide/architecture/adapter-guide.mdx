...
Telegram 使用 `nonebot-adapter-telegram`，通过
...
默认使用 long polling；Webhook 需要 HTTPS 入口及适配器对应配置。受限网络
...
| `PlatformAdapterUnknownError` | 在 `LINGCHUAdapter` 中指定了 Lingchu 未实现或无法识别的适配器 ID |

<Aside type="caution" title="启动失败">
