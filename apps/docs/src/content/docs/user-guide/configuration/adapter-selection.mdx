...
Lingchu Bot groups concrete adapters into platform capabilities. Only one
adapter is enabled per platform. Supported adapter IDs are:
...
The backward-compatible default is OneBot V11. Select `~telegram` explicitly
for a Telegram-only deployment, or select both IDs with `+` when both adapters
are loaded. Other adapter IDs (such as Milky, QQ, or OneBot V12) are not
implemented; configuring them raises `PlatformAdapterUnknownError`.
...
<Aside type="caution" title="Conflict prevention">
...
For developer documentation on the adapter registry and adding new platforms, see [Adapter Guide](/developer-guide/architecture/adapter-guide).
