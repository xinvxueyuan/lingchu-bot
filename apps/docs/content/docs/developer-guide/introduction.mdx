...
同一平台显式配置多个已知适配器会抛出 `PlatformAdapterConflictError`。未配置时，如果运行时同时注册多个已知 QQ 适配器，也会启动失败。显式选择一个 QQ 适配器后，其他同平台适配器会被视为不存在，不参与平台识别、消息存储或 API 调用记录。其他平台 profile 和非群管理能力以后续实现和测试为准。
