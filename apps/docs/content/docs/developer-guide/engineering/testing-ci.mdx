...
CI runs tests on six database backends:
...
- **SQL Server** (`mcr.microsoft.com/azure-sql-edge:latest` service container + apt `msodbcsql18`)

The `fail-fast: false` strategy ensures all databases are tested even if one fails. Oracle / SQL Server start slowly (health-start-period 90-180s); a full matrix run takes roughly 5-10 minutes.
