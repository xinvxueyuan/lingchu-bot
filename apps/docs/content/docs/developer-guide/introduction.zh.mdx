...
| `repositories/user_mapping.py` | UID 与平台账号绑定的兼容入口 |
| `services/messagestore.py` | 消息事件、处理结果、Bot 生命周期和平台 API 调用摘要记录 |
| `start/` | 引导、初始化和启动钩子 |

## 适配器状态

插件按平台能力声明适配器，而不是把所有导入的具体适配器都视为可用。当前已实现 QQ 平台 profile，启动流程中仅 `~onebot.v11` 可用。Milky（`~milky`）、QQ 官方（`~qq`）和 OneBot V12（`~onebot.v12`）已停维并从启动流程中移除，源代码保留供 `tools/adapter_loader.py` 按需加载。未配置时默认启用 OneBot V11。
