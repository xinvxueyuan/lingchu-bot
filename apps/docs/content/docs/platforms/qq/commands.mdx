...
- `speech-management` / `发言管理`: mute/unmute member, enable/disable whole-group mute.
...
- `bot-control` / `机器人控制`: silence/speak (silent mode), boot/shutdown (handle gate).
...
Blocklist commands are available on OneBot V11 only. Blocking stores the user in the group or global blocklist, kicks the user from the current group, and rejects future group join requests while the block is active.
...
English: `block`, `global-block`, `unblock`, `global-unblock`, `clear-blocklist`, `global-clear-blocklist`.

- Duration default: permanent.
- Block reason default: `违反群规「默认」`.
- Unblock/clear reason default: `管理员操作「默认」`.
...
Remote announcement requires `LLOneBot >= 7.12.0` or `NapCat.Onebot >= 4.18.0`. Other OneBot V11 implementations cannot send group notices. The remote announcement menu entry is hidden when the implementation is unsupported.
