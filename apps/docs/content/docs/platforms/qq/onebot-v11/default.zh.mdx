...
`default` 包含只依赖 OneBot V11 标准 API 接口的处理器。这些处理器可以在任何 OneBot V11 实现（NapCat、LLOneBot、Lagrange 等）上运行，无需实现专属扩展。

## 可用命令

[QQ 命令](../commands) 中列出的所有命令在默认实现中都可用，除了：

- `发送群公告` / `send-announcement` — 受实现版本门控（见 [NapCat](./napcat) 和 [LLOneBot](./llonebot)）
