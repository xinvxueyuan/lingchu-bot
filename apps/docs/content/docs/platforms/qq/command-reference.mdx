...
```text
novelai-image <description>
```

The removable `core/subplugins/novelai_image` child plugin first converts the
user description into an English scene description and NovelAI tags through its
dedicated LLM settings, then generates one V4.5 image. Chinese locales use
`生图 <描述>` instead. Installing or removing the child directory takes effect
