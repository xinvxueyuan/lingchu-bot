...
- 数量默认读取 `config.json5` 的 `recall_message_default_count`（默认 `10`）。
- 单个数字参数会被视为数量：`撤回 20`。
...
- 系统提示词：默认 `"你是一个友好的群聊助手。"`，可通过 `chat.json5` 的 `defaults.system_prompt` 自定义。
- 功能开关：`chat.json5` 的 `enabled` 字段控制启用/禁用。

<Callout type="info" title="LLM 配置">

聊天命令依赖运行时 LLM 配置。在 `config.json5` 或环境变量中设置：
