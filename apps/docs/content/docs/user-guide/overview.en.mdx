...
- Startup fails when multiple QQ adapters are registered without an explicit selection. After one adapter is selected explicitly, other same-platform adapters are treated as absent.
