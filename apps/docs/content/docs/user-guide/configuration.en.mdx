...
Only the QQ platform profile is implemented at the moment, so `~telegram` and `~discord` are ignored in this example, while QQ selects `~onebot.v11`. To enable Milky:
