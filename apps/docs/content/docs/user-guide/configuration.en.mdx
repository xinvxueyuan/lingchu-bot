...
Only the QQ platform profile is implemented at the moment, so `~telegram` and `~discord` are not treated as enabled known adapters in this example, while QQ selects `~onebot.v11`. To enable Milky:
...
When `LINGCHUAdapter` is unset, runtime registration of multiple known QQ adapters also fails startup. Once one QQ adapter is configured explicitly, other same-platform adapters may still be imported or registered, but Lingchu Bot treats them as absent. They do not participate in platform resolution, message storage, or API call recording.
