...
Feedback text uses `LINGCHU_LOCALE`, `lc_locale`, or `locale` from the current NoneBot configuration. Without configuration, the default is `zh_CN`. The current repository provides `zh_CN` and `en_US` catalogs. See [Internationalization](../developer-guide/i18n.mdx).
