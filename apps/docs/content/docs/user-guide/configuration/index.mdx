...
The nullable `prompt_llm_provider`, `prompt_llm_model`,
`prompt_llm_base_url`, `prompt_llm_api_key`, and `prompt_llm_timeout` settings
fall back independently to the matching parent `ai_*` setting. Their environment
names use the `LINGCHU_NOVELAI_` prefix.
