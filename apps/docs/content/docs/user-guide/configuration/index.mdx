...
<TypeTable
  type={{
    ...
    ai_provider: {
      type: '"litellm" | "openai"',
      default: '"litellm"',
      description: "LLM SDK backend; OpenAI is kept as the direct SDK fallback",
    },
    ai_model: {
      type: "string",
      default: '"gpt-4o-mini"',
      description: "Default chat completion model for the selected AI provider",
    },
    ai_base_url: {
...
The prompt pipeline uses the global user `ai_*` profile for language
understanding, translation, hint extraction, and optional provider-native web
search. This is a breaking configuration change: all former child-specific LLM
keys have been removed and are ignored. Delete them from existing
`novelai_image.toml` files and configure the global `ai_provider`, `ai_model`,
`ai_base_url`, `ai_api_key`, and `ai_timeout` settings instead.
